<template>
  <button
    @click="handleClick"
    class="ui-button"
    :class="[`ui-button--${type}`, {'is-plain': plain}, {'is-round': round}, {'is-circle': circle}]"
  >
    <span>
      <slot></slot>
    </span>
...
export default {
  name: 'UiButton',
  // 封装一个通用的组件，会对 props 做一个约束， props 进行校验
  props: {
    type: {
      type: String,
      default: 'default'
...
// 让图标和文字之间空开
.ui-button [class*='ui-'] + span {
  margin-left: 5px;
}
// 禁用
.ui-button.ui-button.is-disabled .mask {
  position: absolute;
  width: 100%;
  height: 100%;
...
.ui-button.is-round .mask {
  border-radius: 20px;
}

.ui-button.is-circle .mask {
  border-radius: 50%;
}
