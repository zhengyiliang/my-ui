<template>
  <label
    :class="['ui-checkbox',{'is-checked':isChecked},{'is-disabled': disabled},{'is-mimicry': mimicry}]"
  >
...
export default {
  ...
  computed: {
    model: {
      get() {
        return this.CheckboxGroup.value || this.value
      },
      set(value) {
        this.isGroup
...
// 父元素的最后的一个子元素ui-checkbox取消margin-right，也包括嵌套子元素的最后一个ui-checkbox
.ui-checkbox:last-of-type {
  margin-right: 0;
}
...
.ui-checkbox.is-disabled {
  ...
  span.ui-checkbox__label {
    color: #c0c4cc;
    cursor: not-allowed;
  }
}

.ui-checkbox.is-mimicry {
  .ui-checkbox__inner {
